    ...
    <mat-dialog-content class="mat-dialog-content">
      @if (loading) {
        <div class="flex justify-center items-center py-8">
          ...
          <span class="ml-4">Cargando información de Teams...</span>
        </div>
      } @else if (teamsData) {
        <mat-card class="mb-4">
          <mat-card-content class="pt-4">
            <div class="grid grid-cols-1 gap-4">
              <!-- Instrucciones de Teams -->
                ...
                <div class="flex items-start gap-3 mb-3">
                  ...
                  <div>
                    <p class="font-semibold text-[var(--mat-sys-primary)] mb-2">
                      Instrucciones para conectarse a Teams:
                    </p>
                    <ol
                    ...
                    >
                      ...
                      <li>Ingrese su cuenta (email de abajo)</li>
                      <li>
                        Si aún no cambió su contraseña de Teams, use la
                        contraseña mostrada abajo
                      </li>
                      <li class="text-[var(--mat-sys-error)] font-medium">
                        Si sale error, inténtelo desde otro navegador o reinicie
                        su PC
                      </li>
                    </ol>
                  </div>
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center gap-2" style="flex-wrap: wrap;">
                    <p class="text-lg email-text">
                      {{ teamsData.user.mail || "N/A" }}
                      <button
                        mat-icon-button
                        (click)="copiarTexto(teamsData.user.mail)"
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center gap-2">
                    <p class="text-lg font-mono">
                      Cultural{{ user.userName?.substring(0, 4) }}
                      <button
                        mat-icon-button
                        (click)="
                          copiarTexto(
...
export class TeamsDialogComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { idteams: string },
  ) {}
  ...
  cargarDatosTeams(): void {
    if (!this.data.idteams) {
      this.loading = false;
      this.errorMessage = "ID de Teams no válido";
      return;
    }

    this.alumnoService.obtenerCuentaTeams(this.data.idteams).subscribe({
      next: (response) => {
        this.teamsData = response;
        this.loading = false;
      },
      error: (error) => {
        this.errorMessage = "Error al cargar la información de Teams";
        this.loading = false;
        this.snack.danger("Error al cargar la información de Teams");
      },
    });
  }

    ...
    <div class="page-container">
      ...
      @if (libros() && libros().length > 0) {
        <mat-card class="libros-section" style="margin-bottom: 2rem;">
          <mat-card-header>
            ...
            <mat-card-subtitle>Material académico para tu curso - {{getTipoSeleccionado()}}</mat-card-subtitle>
          </mat-card-header>
          <mat-card-content>
            <!-- Información de productos -->
            <div class="selection-info">
              <mat-icon>info</mat-icon>
              <span>Selecciona los libros que deseas comprar ({{getLibrosSeleccionados().length}} de {{libros().length}} seleccionados)</span>
            </div>

            <div class="libros-grid">
              @for (libro of libros(); track libro.idProducto) {
                <mat-card class="libro-card" [class.selected]="isLibroSeleccionado(libro.idProducto)">
                  ...
                  <mat-card-header>
                    ...
                    <mat-card-subtitle>Código: {{libro.codigoProducto}}</mat-card-subtitle>
                  </mat-card-header>
                  <mat-card-content>
                    <div class="libro-info">
                      ...
                      <div class="precio">
                        <span class="precio-valor">S/ {{libro.precioVenta}}</span>
                      </div>
                    </div>
                  </mat-card-content>
                </mat-card>
        ...
        <mat-card class="resumen-compra-card" style="margin-bottom: 2rem;">
          ...
          <mat-card-content>
            <div class="resumen-content">
              <div class="libro-seleccionado">
                @for (libro of getLibrosSeleccionados(); track libro.idProducto) {
          ...
          <mat-card-actions class="centered-actions">
            <button mat-flat-button 
                    ...
                    [disabled]="procesandoPago || formularioIziPayAbierto || getLibrosSeleccionados().length === 0">
              <mat-icon>payment</mat-icon>
              {{ procesandoPago ? 'Procesando...' : 'PAGAR S/ ' + getPrecioTotal() }}
            </button>
          </mat-card-actions>
        </mat-card>
        }
      ...
      @if (paymentMessage) {
        <div class="payment-message" [ngClass]="paymentMessageClass">
          ...
          @if (paymentMessageClass === 'payment-error' && paymentMessage.includes('pasarela')) {
            <div class="retry-section">
              <button mat-raised-button color="accent" (click)="procesarPago()" [disabled]="procesandoPago || formularioIziPayAbierto">
        ...
        <mat-card class="no-libros-card">
          <mat-card-content>
            <div class="no-libros-content">
              ...
              <p>No se encontraron libros del tipo seleccionado para tu curso en esta sede.</p>
            </div>
          </mat-card-content>
        </mat-card>
      }
...
export class CompraLibrosComponent implements OnInit {
  ...
  cargarLibros(registro: any): void {
    this.libroService.obtenerLibroUltimoCurso(registro.idCurso, registro.idSede)
      .subscribe((libros: any[]) => {
        this.libros.set(libros);
        // Por defecto, seleccionar todos los libros
        const todosSeleccionados = new Set(libros.map(libro => libro.idProducto));
  ...
  procesarPago(): void {
    ...
    const orderNumber = this.periodoActual().idPeriodo + this.librosSeleccionados().values().next().value + this.user.userName;
    
    this.registroService.obtenerTokenPago(total, transactionId, orderNumber).subscribe({
      next: (tokenResponse: any) => {
        if (tokenResponse.code === '00') {
          this.initializeIziPay(tokenResponse, total, transactionId, orderNumber);
        } else {
  ...
  private initializeIziPay(tokenResponse: any, amount: number, transactionId: string, orderNumber: string): void {
  ...
  private setupIziPayForm(tokenResponse: any, amount: number, transactionId: string, orderNumber: string): void {
    ...
    try {
      ...
      checkout.LoadForm({
        ...
        callbackError: (error: any) => {
          this.formularioIziPayAbierto = false;
          this.showPaymentMessage('Error en el procesamiento del pago. La pasarela de pagos no está disponible temporalmente.', 'error');
        }
      });
      ...
      setTimeout(() => {
        if (this.formularioIziPayAbierto) {
          this.formularioIziPayAbierto = false;
          this.showPaymentMessage('El tiempo de espera para el pago ha expirado. Por favor, intente nuevamente.', 'error');
        }
      }, 300000); // 5 minutos
  ...
  private handlePaymentResponse(response: any): void {
    ...
    if (!response || typeof response !== 'object') {
      this.showPaymentMessage('Error: Respuesta inválida del sistema de pagos. Por favor, contacte soporte.', 'error');
      return;
    }
    
    if (response.code === '00') {
      this.showPaymentMessage('¡Pago exitoso! Registrando tu compra...', 'success');
      
      // Registrar la compra de todos los libros
      this.registrarCompraLibros(response);
    } else if (response.code === '504' || response.message?.includes('timeout')) {
      this.showPaymentMessage('La pasarela de pagos no está respondiendo. Por favor, intente nuevamente en unos minutos.', 'error');
    } else {
      const errorMessage = response.message || response.error || 'Error desconocido en el procesamiento del pago';
  ...
  private registrarCompraLibros(paymentResponse: any): void {
    ...
    const compraLibrosDto = {
      ...
      pCompra: libros.reduce((total, libro) => total + (libro.precioCompra || 0), 0),
    ...
    this.libroService.registrarLibro(compraLibrosDto).subscribe({
      next: (boletaData: any) => {
        this.showPaymentMessage('¡Compra registrada exitosamente! Generando boleta...', 'success');
      ...
      error: (error) => {
        ...
        this.showPaymentMessage('Pago exitoso, pero hubo un error al registrar la compra. Contacte soporte.', 'error');

      ...
      @if (libroData()) {
        <mat-card class="resumen-card" >
          <mat-card-header>
            ...
            <mat-card-subtitle>Revisa los detalles antes de proceder con el pago</mat-card-subtitle>
          </mat-card-header>
          <mat-card-content>
            <mat-list>
              <mat-list-item>
          ...
          <mat-card-actions class="actions-container">
            ...
            <button mat-flat-button
              ...
              {{ procesandoPago ? 'Procesando...' : 'PAGAR S/' + libroData()?.precioVenta }}
            </button>
          </mat-card-actions>
        </mat-card>
        ...
        <mat-card class="error-card">
          <mat-card-content>
            <div class="error-content">
              ...
              <h3>Error</h3>
              <p>No se encontraron datos del libro. Por favor, regresa y selecciona un libro nuevamente.</p>
...
export class ResumenCompraLibroComponent implements OnInit {
  ...
  procesarPago(): void {
    ...
    this.registroService.obtenerTokenPago(libro.precioVenta, transactionId, orderNumber).subscribe({
      next: (tokenResponse: any) => {
        if (tokenResponse.code === '00') {
          this.initializeIziPay(tokenResponse, libro.precioVenta, transactionId , orderNumber);
        } else {
  ...
  private initializeIziPay(tokenResponse: any, amount: number, transactionId: string , orderNumber): void {
  ...
  private setupIziPayForm(tokenResponse: any, amount: number, transactionId: string , orderNumber): void {
    ...
    const iziConfig = {
      config: {
        ...
        billing: {
          firstName: firstName,
          lastName: lastName,
          email: user?.correo || 'test@example.com',
